...
describes what users get. Pick a format with `-o`, or let bioproject choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bioproject <command> -o table   # aligned columns for reading
bioproject <command> -o jsonl   # one JSON object per line, for piping
bioproject <command> -o json    # a single JSON array
bioproject <command> -o csv     # spreadsheet friendly
bioproject <command> -o tsv     # tab-separated
bioproject <command> -o url     # just the URL column
bioproject <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bioproject <command> --fields id,title,url
```
...
```bash
bioproject <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bioproject <command>            # a table, because this is a terminal
bioproject <command> | wc -l    # JSONL, because this is a pipe
```
